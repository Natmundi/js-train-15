...
function generateRandomPassword(length) {
  ...
  for (let i = 0; i < length; i++) {
    ...
    const symIndex = characters.charAt(randomIndex);
    password += symIndex;
  }
  return password;
}
...
function roundObjectValues(obj) {
  ...
  const getEnter = Object.entries(obj);
  const checkEnter = getEnter.map(([key, value]) => {
  ...
  const convertedObj = Object.fromEntries(checkEnter);
  return convertedObj;
}
...
function calculateVolumeCylinder(radius, height) {
  ...
  const volum = Math.PI * Math.pow(radius, 2) * height;
  const round = Math.ceil(volum);
  return round;
}
...
function getFractionalPart(num) {
  ...
  const frac = num - trunc;
  const result = Math.fround(frac);
  return result;
}
...
function isTotalPriceExceedsMaxPrice(products, maxPrice) {
  ...
  const convertedTotalP = Math.fround(totalPrice);
  const convertedMaxP = Math.fround(maxPrice);
  return convertedTotalP > convertedMaxP;
}
